...
export default function AdminLogin({ adminEmail, adminPass, adminBusy, adminErr, onAdminEmail, onAdminPass, onAdminLogin, onAdminForgot, goHome }) {
  const accent = '#D6C3A0';
  const canSubmit = (adminEmail || '').trim() && adminPass && !adminBusy;
  const pole = {
      ...
      <div style={{ width: '100%', maxWidth: '400px', animation: 'gbfade 0.5s ease both' }}>
        ...
        <div style={{ background: '#15130F', border: '1px solid #2A2622', borderRadius: '16px', padding: '24px' }}>
          ...
          <button onClick={submit} disabled={!canSubmit}
            style={{ width: '100%', cursor: canSubmit ? 'pointer' : 'not-allowed', opacity: canSubmit ? 1 : 0.55, background: accent, color: '#0E0E0E', border: 'none', borderRadius: '10px', padding: '14px', fontFamily: "'Oswald'", fontWeight: '600', letterSpacing: '0.06em', textTransform: 'uppercase', fontSize: '15px' }}>{adminBusy ? 'Signing in…' : 'Sign in'}</button>
          <button onClick={() => onAdminForgot(adminEmail)} disabled={adminBusy}
            style={{ display: 'block', width: '100%', marginTop: '12px', background: 'transparent', border: 'none', color: '#9A9388', fontSize: '13px', cursor: adminBusy ? 'not-allowed' : 'pointer', textDecoration: 'underline', textUnderlineOffset: '3px' }}>Forgot password?</button>
